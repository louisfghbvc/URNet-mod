...
class URN2(nn.Module):
    def __init__(self, args):
        ...
        for p in range(4):
            if p == 3:
                down.append(RFDBBlock(nf // channels[p], nf // channels[p], ver=True, tail=True, shuffle=True))
            else:
                down.append(RFDBBlock(nf // channels[p], nf // channels[p + 1], ver=True, shuffle=True))
